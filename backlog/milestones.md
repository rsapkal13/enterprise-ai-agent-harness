# Milestones

## v0.1 - Spec Release

- Repository foundation
- Public docs
- Draft schemas
- Telco sample journey
- System, context, audit, and evaluation object coverage
- Initial backlog

## v0.2 - Local Runtime Preview

- Schema loader
- In-memory registries
- Mock tool gateway
- Workflow simulation

## v0.3 - Policy and Workflow

- Policy evaluation contract
- Human approval states
- Workflow rollback model
- Audit obligations

## v0.4 - Observability

- Trace model
- Audit log writer
- Evaluation results
- Local reports

## v0.5 - Demo Experiences

- Demo runner
- Documentation site
- Control plane dashboard phases A and B: static registry browser, policy inspector, audit trail viewer
- Website aligned with v0.1 scope and future-state roadmap

## v0.6 - Runtime-Connected Dashboard

- Journey monitor connected to local runtime (phase C)
- Evaluation console (phase D)
- Trace-linked audit events
- Runtime package naming and publishing strategy
- CLI scope for validation and local fixture workflows

## v0.7 - Approval Queue

- Human-in-the-loop approval queue in the dashboard (phase E)
- Approval write model via policy engine API
- Approval audit log
- Scope-change and recertification workflow design

## v0.8 - Agent Directory and Controlled Delegation

- Agent capability card specification
- Allow-listed agent discovery model
- Policy-gated handoff pattern
- Audit events for agent-to-agent delegation
- Maturity guidance for controlled agent collaboration

## Post-v0.8 - Deployment and Managed Operations

- Self-hosted deployment guidance
- Managed control-plane considerations
- Authentication and role-based access model
- Multi-tenant operating model
